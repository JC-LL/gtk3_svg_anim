...
class CompleteSVGAnimatorWithControls
  ...
  def setup_gui
    ...
    @window.signal_connect('destroy') { quit_application }
    ...
    # Contrôle de lecture
    playback_box = Gtk::Box.new(:horizontal, 5)

    # Bouton Step (pas à pas)
    @step_button = Gtk::Button.new(:label => "⏭ Step")
    @step_button.signal_connect('clicked') { step_animation }
    playback_box.pack_start(@step_button, :expand => true, :fill => true, :padding => 2)

    # Bouton Play/Pause
    @play_button = Gtk::Button.new(:label => "▶ Play")
    @play_button.signal_connect('clicked') { toggle_animation }
    playback_box.pack_start(@play_button, :expand => true, :fill => true, :padding => 2)

    control_panel.pack_start(playback_box, :expand => false, :fill => true, :padding => 5)
    ...
    # Contrôle du pas
    step_box = Gtk::Box.new(:vertical, 5)
    step_label = Gtk::Label.new("Taille du pas:")
    step_box.pack_start(step_label, :expand => false, :fill => false, :padding => 2)

    step_buttons_box = Gtk::Box.new(:horizontal, 5)

    small_step_button = Gtk::Button.new(:label => "Petit")
    small_step_button.signal_connect('clicked') { @step_size = 0.02; update_step_label }
    step_buttons_box.pack_start(small_step_button, :expand => true, :fill => true, :padding => 2)

    medium_step_button = Gtk::Button.new(:label => "Moyen")
    medium_step_button.signal_connect('clicked') { @step_size = 0.05; update_step_label }
    step_buttons_box.pack_start(medium_step_button, :expand => true, :fill => true, :padding => 2)

    large_step_button = Gtk::Button.new(:label => "Grand")
    large_step_button.signal_connect('clicked') { @step_size = 0.1; update_step_label }
    step_buttons_box.pack_start(large_step_button, :expand => true, :fill => true, :padding => 2)

    step_box.pack_start(step_buttons_box, :expand => false, :fill => true, :padding => 2)
    control_panel.pack_start(step_box, :expand => false, :fill => true, :padding => 5)

    # Affichage de la taille du pas actuelle
    @step_size = 0.05
    @step_label = Gtk::Label.new("Pas: #{(@step_size * 100).round(1)}%")
    control_panel.pack_start(@step_label, :expand => false, :fill => false, :padding => 5)
    ...
    separator.margin_top = 20
    separator.margin_bottom = 10
    control_panel.pack_start(separator, :expand => false, :fill => true, :padding => 5)

    # Bouton Quitter
    @quit_button = Gtk::Button.new(:label => "🚪 Quitter")
    @quit_button.signal_connect('clicked') { quit_application }
    control_panel.pack_start(@quit_button, :expand => false, :fill => true, :padding => 5)
  ...
  def step_animation
    ...
    @drawing_area.queue_draw
    update_status_label
    @status_label.label += " (Step)"
  end

  def update_step_label
    @step_label.label = "Pas: #{(@step_size * 100).round(1)}%"
  end

  def update_status_label
    @status_label.label = @is_playing ? "En cours: #{@time.round(2)}s" : "Arrêté: #{@time.round(2)}s"
  end
  ...
  def quit_application
    ...
    GLib::Source.remove(@animation_id) if @animation_id

    # Fermer toutes les fenêtres
    Gtk.main_quit

    # Message de confirmation (optionnel)
    puts "Application fermée proprement."
  end
