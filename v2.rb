...
class CompleteSVGAnimatorWithControls
  ...
  def setup_gui
    # Fenêtre principale
    @window = Gtk::Window.new("Animation SVG Ruby avec Contrôles")
    ...
    # Zone de dessin (80% de l'espace)
    @drawing_area = Gtk::DrawingArea.new
    drawing_frame = Gtk::Frame.new
    drawing_frame.set_size_request(800, 600)
    drawing_frame.add(@drawing_area)
    main_box.pack_start(drawing_frame, :expand => true, :fill => true, :padding => 5)

    # Panneau de contrôle vertical (20% de l'espace)
    control_panel = Gtk::Box.new(:vertical, 10)
    control_panel.set_size_request(180, 600)
    control_panel.halign = :center
    control_panel.valign = :start
  ...
  def take_screenshot
    ...
    if dialog.run == :accept
      ...
      surface = Cairo::ImageSurface.new(:argb32, 800, 600)
      cr = Cairo::Context.new(surface)

      # Dessiner le SVG animé
      draw_animated_svg(cr)

      # Sauvegarder en PNG
      surface.write_to_png(filename)

      @status_label.label = "Capture sauvegardée: #{File.basename(filename)}"
    end
